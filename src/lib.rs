...
fn init_sdl() -> napi::Result<()> {
    unsafe {
        INIT.call_once(|| {
            ...
            let (freq, format, channels) = sdl2::mixer::query_spec()
                .expect("Failed to query audio spec");
            println!("Audio specs - frequency: {}, format: {:?}, channels: {}", freq, format, channels);
...
impl Mixer {
    ...
    pub fn load_wav(&self, path: String) -> napi::Result<Chunk> {
        let chunk = sdl2::mixer::Chunk::from_file(&path)
            .map_err(|e| {
                println!("Error loading WAV: {}", e);
                napi::Error::from_reason(e.to_string())
            })?;
        Ok(Chunk { inner: Arc::new(chunk) })
    }
    ...
    pub fn load_music(&self, path: String) -> napi::Result<Music> {
        ...
        Ok(Music { inner: Arc::new(static_music) })
    }
